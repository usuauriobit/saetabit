<div class="p-4 my-3 bg-gray-50 rounded-box">
    ...
    @forelse ($vuelo->incidencias_avion as $incidencia)
        <x-master.item label="Avión que sufrió la incidencia" sublabel="{{optional($incidencia->avion_before)->matricula}}">
            ...
            <x-slot name="actions">
                <button wire:loading.attr="disabled"  class="mt-4 btn btn-danger btn-sm" wire:click="deleteIncidenteAvion('{{$incidencia->id}}')">
    ...
    @empty
        @if ($is_active)
            ...
            @else
                <livewire:intranet.comercial.vuelo.components.input-avion min-asientos="{{ $vuelo->nro_asientos_ofertados }}" emitEvent="avionIncidenteSelected" />
            @endif

            <x-master.input label="Descripcion de la incidencia" name="form.descripcion" wire:model="form.descripcion" />
            <button class="w-full mt-3 btn btn-primary" wire:loading.attr="disabled"  wire:click="saveAvion"> <i class="la la-save"></i> Guardar</button>
        @else
