...
class BaseGenerator{
    ...
    public function getColumnsFromModel(){
        ...
        $columns = [];
        $columns_db = \DB::select("SELECT * FROM  information_schema.columns WHERE table_name = '{$model->getTable()}'");
        // $columns_db = collect($columns_db)->map(function($value, $key) { return $value->column_name; });
        // dd($columns_db);
        $fillables = $model->getFillable();
        foreach ($columns_db as $column) {
            if( in_array($column->column_name, $fillables)){
                // dd($column);
                $columns[] = [
                    'field'     => $column->column_name,
                    'type'      => $column->udt_name,
                    'title'     => str_replace('_', ' ',$column->column_name),
                    'is_null'   => $column->is_nullable == 'YES',
                    'label'     => str_replace('_', ' ', ucfirst(str_replace('_id', '',$column->column_name))),
                    'is_numeric'=> $this->isNumeric($column->udt_name),
                    'is_date'   => $this->isDate($column->udt_name),
                    'is_unique' => $this->isUnique($column->udt_name),
                    'foreign_status' => $this->getForeignStatus($column->column_name),
                ];
            }
        }
        return $columns;
    ...
    public function isNumeric($type){
        return strpos($type, 'int') || strpos($type, 'decimal');
    }
    public function isDate($type){
        return strpos($type, 'date') || strpos($type, 'timestamp');
    }
    public function isUnique($type){
        // return strpos($type, 'unique');
        return false;
    }
    ...
    public function getViewPath($file_name){
        $path = '/views/livewire/';
        if($this->subfolder) $path .= str_replace('.','/', $this->subfolder).'/';
    ...
    public function getControllerPath($file_name){
        ...
        if($this->subfolder) $path .= ucfirst(\Str::camel( str_replace( ' ', '\\', ucwords(str_replace('.', ' ', $this->subfolder)) ) )).'/';
        $path .= $file_name;
        ...
        foreach((new ReflectionClass($model))->getMethods(ReflectionMethod::IS_PUBLIC) as $method)
        {
            ...
            try {
                ...
                if ($return instanceof Relation) {
                    if(
                        !in_array( $method->getName(), ['user_created' ,'user_updated' ,'user_deleted'] )
                        && !in_array( (new ReflectionClass($return))->getShortName(), ['MorphOne', 'MorphMany'] )
                    )
    ...
    public function getNamespace(){
        ...
        if($this->subfolder){
            $subfolder = ucfirst(\Str::camel( str_replace(' ', '\\', ucwords(str_replace('.', ' ', $this->subfolder))) ));
